...
for word in words:
    ...
    for letter in range(1, len(word)):
        distance += math.dist((xs[letters.index(word[letter])], ys[letters.index(word[letter])]), (xs[letters.index(word[letter - 1])], ys[letters.index(word[letter - 1])]))
    score = (distance / (len(word) - 1)) * 100 / 18
